...
const HEADING_COLOR  = '#1a4a1a';   // dark green for titles and labels
const BODY_COLOR     = '#111111';   // near-black for body text
const DIM_COLOR      = '#555555';   // grey for meta / footer
const BG_COLOR       = '#ffffff';   // white page background
const HEADER_BG      = '#e8f5e8';   // very light green header bar
const RULE_COLOR     = '#2e7d32';   // medium green for divider lines
    ...
    try {
      ...
      doc.on('pageAdded', () => {
        ...
        if (pageNum === 1) {
          ...
          doc.rect(0, 0, width, HEADER_H).fill(HEADER_BG);
          doc
            .font('Courier-Bold').fontSize(16).fillColor(HEADING_COLOR)
            .text('STRANGE RECIPE GENERATOR', ML, 16, { width: W, align: 'center' });
          doc
            .font('Courier').fontSize(8).fillColor(DIM_COLOR)
            .text(
              '// FUSION CUISINE AI SYNTHESIS ENGINE — OFFLINE LOCAL BUILD //',
              ML, 38, { width: W, align: 'center' }
            );
          doc
            .moveTo(ML, HEADER_H - 2).lineTo(width - MR, HEADER_H - 2)
            .strokeColor(RULE_COLOR).lineWidth(0.75).stroke();
        }

        // Reset fill/stroke so overflow text from an interrupted text() call
        // renders in the correct colour on the new page.
        doc.fillColor(BODY_COLOR).strokeColor(HEADING_COLOR).lineWidth(1);
      });
      ...
      for (const line of lines) {
        ...
        if (isTitle) {
          doc.moveDown(0.5);
          doc
            .font('Courier-Bold').fontSize(11).fillColor(HEADING_COLOR)
            .text(trimmed, MARGIN.left, doc.y, { width: W });
          const lineY = doc.y + 2;
          doc
            .moveTo(MARGIN.left, lineY).lineTo(MARGIN.left + W, lineY)
            .strokeColor(RULE_COLOR).lineWidth(0.4).stroke();
          doc.moveDown(0.3);

        } else if (isMeta) {
          const colon = trimmed.indexOf(':');
          doc
            .font('Courier-Bold').fontSize(10).fillColor(HEADING_COLOR)
            .text(trimmed.slice(0, colon + 1), MARGIN.left, doc.y, { continued: true, width: W });
          doc
            .font('Courier').fontSize(10).fillColor(BODY_COLOR)
